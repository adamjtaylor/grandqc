...
MODEL_QC_DIR = args.MODEL_QC_DIR
...
REPORT_FILE_NAME = (
    f"report_{case_name}_" + str(start) + "_" + str(end)
...
for slide_name in slide_names[start:end]:
    try:
        ...
        # GET SLIDE INFO
        p_s, patch_n_w_l0, patch_n_h_l0, mpp, w_l0, h_l0, obj_power = slide_info(
            slide, M_P_S_MODEL, MPP_MODEL
        )
        ...
        overlay = make_overlay(
            slide, map, p_s, patch_n_w_l0, patch_n_h_l0, OVERLAY_FACTOR
        )
        ...
        output_temp = (
            output_temp
            + str(patch_n_h_l0 * p_s)
            + "\t"
            + str(patch_n_w_l0 * p_s)
            + "\t"
        )
