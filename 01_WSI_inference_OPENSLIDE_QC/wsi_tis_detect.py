...
# COLORS for MASK
colors = [[50, 50, 250], [128, 128, 128]]  # BLUE: TISSUE  # GRAY: BACKGROUND
...
MODEL_TD_DIR = args.MODEL_TD_DIR
...
for slide_name in slide_names:
    ...
    try:
        ...
        image_or = slide.get_thumbnail(
            (w_l0 // reduction_factor, h_l0 // reduction_factor)
        )
        image_or.save(tis_det_dir_thumb + slide_name + ".jpg", quality=80)
    except Exception as e:
        print(f"Exception with loading {slide_name}: {e}")
        exit(1)
    ...
    try:
        ...
        print(
            "Overhang (< 1 patch) for width and height: ", overhang_wi, ",", overhang_he
        )

    except Exception as e:
        print(f"Exception with jpeg compression for {slide_name}: {e}")
        exit(1)
    ...
    try:
        ...
        print(f"Exception with inference for {slide_name}: {e}")
        exit(1)

    try:

        Image.fromarray(end_image).save(
            os.path.join(tis_det_dir_mask, slide_name + "_MASK.png")
        )
        Image.fromarray(end_image_class_map).save(
            os.path.join(tis_det_dir_mask_col, slide_name + "_MASK_COL.png")
        )
        overlay = cv2.addWeighted(
            np.array(image), OVER_IMAGE, end_image_class_map, OVER_MASK, 0
        )
        overlay = Image.fromarray(overlay)
        overlay.save(os.path.join(tis_det_dir_over, slide_name + "_OVERLAY.jpg"))
    except Exception as e:
        print(f"Exception with saving outputs for {slide_name}: {e}")
        exit(1)
