...
parser.add_argument(
    "--mpp", dest="mpp", help="mpp of the slide", default=0.2425, type=float
)
...
for slide_name in slide_names:
    try:
        ...
        image_or = cv2.resize(
            image_or,
            (int(w_l0 // reduction_factor), int(h_l0 // reduction_factor)),
            interpolation=cv2.INTER_CUBIC,
        )
        Image.fromarray(image_or).save(
            os.path.join(tis_det_dir_thumb, slide_name + ".jpg"), quality=80
        )
